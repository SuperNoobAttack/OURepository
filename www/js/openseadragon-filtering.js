...
(function() {
    ...
    $.Filters = {
        ...
        //https://ncit.s3.amazonaws.com/s3fs-public/documents/files/Using-Color-Infrared-Imagery-20110810.pdf
        //CIR:
        // NIR -> red
        // red -> green
        // gren -> blue
        ...
        CNDVI: function () {
            return function(context, callback) {
                ...
                for (var i = 0; i < pixels.length; i += 4) {
                    ...
                    var ndvi = Math.trunc(((nir - red) / (nir + red) + 1.0) * (127.5));
                    pixels[i] = ndvi;
                    pixels[i + 3] = 255;
                }
                context.putImageData(imgData, 0, 0);
        ...
        VARI: function () {
            return function(context, callback) {
                ...
                for (var i = 0; i < pixels.length; i += 4) {
                    ...
                    var green = pixels[i + 1] / 255.0;
                    var blue = pixels[i + 2] / 255.0;
                    var vari = Math.trunc((1.0 + ((green - red) / (green + red - blue))) * 127.5);
